...
public class SubPictureImpl extends AnyPicture implements SubPicture {
  ...
  public SubPictureImpl(Picture source, int xoff, int yoff, int width, int height) {
    ...
    if (xoff < 0 || xoff >= source.getWidth() || yoff < 0 || yoff >= source.getHeight()
        || width < 1 || xoff + width > source.getWidth() || height < 1
  ...
  public SubPicture extract(double _x, double _y, double _tileWidth, double _tileHeight) {
    return new SubPictureImpl(this, x_offset, y_offset, width, height);
  }

  @Override
  public SubPicture extract(double _x, double _y, int _tileWidth, int _tileHeight) {
    return new SubPictureImpl(this, x_offset, y_offset, width, height);
  }
